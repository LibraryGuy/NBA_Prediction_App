...
def load_nba_universe():
    ...
    try:
        ...
        return sos, all_30
    except: return {k: 1.0 for k in all_30.keys()}, all_30

@st.cache_data(ttl=600)
def find_players_fuzzy(name_query):
    all_p = players.get_players()
    return [p for p in all_p if name_query.lower() in p['full_name'].lower()]
...
def get_player_stats(p_id):
    try:
        ...
        return log, info['TEAM_ABBREVIATION'].iloc[0], info['POSITION'].iloc[0]
...
# --- 2. APP LAYOUT ---
st.set_page_config(page_title="Sharp Pro v4.2", layout="wide")
...
with st.sidebar:
    ...
    injury_pos = st.selectbox("Key Player Out", ["None", "Point Guard", "Center", "Wing/Forward"])
    impact_map = {"None": 1.0, "Point Guard": 1.12, "Center": 1.08, "Wing/Forward": 1.05}
    current_impact = impact_map[injury_pos]
    
    mode = st.radio("Mode", ["Single Player", "Team Scout Radar", "Parlay Builder", "Box Score Scraper"])
...
# --- 3. UPDATED SINGLE PLAYER SEARCH ---
if mode == "Single Player":
    search_q = st.text_input("1. Search Player (e.g., 'Sarr' or 'LeBron')", "Sarr")
    matches = find_players_fuzzy(search_q)
    
    if matches:
        # SECOND BAR: Choose from specific results
        player_choice = st.selectbox("2. Confirm Player Choice", matches, format_func=lambda x: x['full_name'])
        p_df, team_abbr, p_pos = get_player_stats(player_choice['id'])
        ...
        if not p_df.empty:
            ...
            with main_col:
                ...
                trend_fig.add_hline(y=current_line, line_dash="dash", line_color="red")
                trend_fig.update_layout(template="plotly_dark", height=250)
                st.plotly_chart(trend_fig, use_container_width=True)

                # RE-ADDED: Efficiency Scatter
                st.subheader("📊 Efficiency Matrix (Usage vs PPS)")
                eff_fig = go.Figure(go.Scatter(x=p_df['usage'].head(15), y=p_df['pps'].head(15), mode='markers+text', text=p_df['points'], marker=dict(size=14, color=p_df['points'], colorscale='Plasma')))
                eff_fig.update_layout(template="plotly_dark", xaxis_title="Usage Volume", yaxis_title="PPS", height=300)
                st.plotly_chart(eff_fig, use_container_width=True)

            with side_col:
                # RE-ADDED: Monte Carlo Poisson
                st.subheader("🎲 Win Probability")
                win_p = (1 - poisson.cdf(current_line - 0.5, sharp_lambda))
                st.metric("Probability", f"{round(win_p * 100, 1)}%")
                ...
                dist_fig.update_layout(template="plotly_dark", height=200, showlegend=False)
                st.plotly_chart(dist_fig, use_container_width=True)
                
                # Kelly Calculation
                dec_odds = 1.91
                k_f = ((dec_odds - 1) * win_p - (1 - win_p)) / (dec_odds - 1)
                st.success(f"**Recommended Stake:** ${max(0, round(k_f * total_purse * kelly_multiplier, 2))}")

    else:
        st.error("No players found. Try a different spelling.")
# ... [Rest of modes: Team Radar, Parlay, Box Score stay the same] ...
