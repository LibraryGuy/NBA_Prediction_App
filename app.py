...
def get_player_data(player_full_name):
    ...
    # Improved Search Logic (Handles hyphens and partials)
    name_clean = player_full_name.lower().replace("-", " ")
    player = [p for p in nba_players if p['full_name'].lower().replace("-", " ") == name_clean]
    if not player:
        player = [p for p in nba_players if name_clean in p['full_name'].lower().replace("-", " ")]
    
    if not player: return pd.DataFrame(), None
    p_id = player[0]['id']
    
    # Try current season, fallback to last season
    log = pd.DataFrame()
    ...
    # SAFETY GATE: Exit if no data was found in either season
    if log.empty:
        return pd.DataFrame(), None

    # This was the crashing line - now protected by the safety gate above
...
# --- 3. UI RENDERING ---
st.title("🏀 NBA Sharp: Auto-Pilot Suite (v3.3)")
sos_data, league_avg_drtg = load_nba_base_data()

with st.sidebar:
    st.header("🎯 Target Selection")
    # Set default to Shai as requested
    ...
    opp_idx = opp_options.index(auto_opp) if auto_opp in opp_options else 0
...
if not p_df.empty:
    ...
    with col_main:
        ...
        trend_fig = go.Figure()
        trend_fig.add_trace(go.Scatter(x=list(range(1, len(last_10)+1)), y=last_10[stat_category], mode='lines+markers', name='Actual Result', line=dict(color='#00ff96', width=3)))
        ...
        trend_fig.update_layout(template="plotly_dark", height=300, xaxis_title="Recent Games", yaxis_title=stat_category.title())
    ...
    st.caption("v3.3 | Shai-Hyphen Fix | KeyError Safety Gate")
else:
    st.warning("No data found for this player. Try adjusting the search name (e.g., 'Shai').")
